...
public class CMAES extends Algorithm {
  ...
  private SolutionSet samplePopulation() throws Exception {
    ...
    for (int iNk = 0; iNk < populationSize; iNk++) {
      ...
      for (int i = 0; i < N; i++) {
        //TODO: Cambiar este aleatorio. Probablemente no sea muy correcto: http://en.wikipedia.org/wiki/CMA-ES
  ...
  private SolutionSet genoPhenoTransformation(double [][] popx) throws Exception {
  ...
  private void updateDistribution() {
    ...
    if (counteval - eigeneval > lambda /(c1+cmu)/N/10) { // to achieve O(N^2)
      ...
      Utils.checkEigenSystem(N, C, diagD, B); // for debugging

      for (int i = 0; i < N; i++) {
        if (diagD[i] < 0) // numerical problem?
          System.out.println("Exception in CMAES.execute(): an eigenvalue has become negative.") ;
  ...
  public SolutionSet execute() throws Exception {
    ...
    while (counteval < maxEvaluations) {
      ...
      System.out.println(counteval + ": " + population_.best(comparator).getObjective(0));
